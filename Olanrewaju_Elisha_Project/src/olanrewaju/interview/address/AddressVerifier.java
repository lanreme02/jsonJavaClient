package olanrewaju.interview.address;
...
import olanrewaju.interview.barrier.Semaphor;
...
public class AddressVerifier {
	...
	private void loadConfigAndInitializeRunParam() {
		...
		try {
			...
			// most like conversion error, but trap everything
			barriar = new Semaphor[maxThreads]; // must be intialized
			e.printStackTrace();
		}
	}
	...
	public ConcurrentLinkedQueue<String> findAddress(
			ConcurrentLinkedQueue<String> _addressList) {
					...
					if (address != null) {
						...
						while (innerLoop && retryCount <= maxRetry) {
							...
							try {// make HTTP call
								...
								/*
								 * Log some connection related error Delay
								 * Thread or discontinue task - not implemented
								 * for this project simply mark the address as
								 * NOT FOUND without a retry
								 */
						   ...
						   if (httpResponseOk) { // check HTTp Response
								try { // parse input
									...
									/*
									 * Log Stream related Error This should be
									 * debugged, no point retrying construct NOTFOUND
									 *  status, Do not retry
									 */
									addressObj = makeUnknowAddress(address);
									String json = HelperUtils
