package olanrewaju.interview.networkUtil;
...
import olanrewaju.interview.address.AddressObject;
...
public class HelperUtils {
	...
	public static String InputStreamToStiring(InputStream inStream) {
		...
		try {
			...
			for (;;) {
				int rsz = input.read(buffer, 0, buffer.length);
				if (rsz < 0)
					break;
	...
	public static ConcurrentLinkedQueue<String> readInputFile(String filePath,
	...
	public static String getConfigProperty(String property) {
		...
		try {
			...
			//input = new FileInputStream("config.properties");
			 ...
			 String filename = "config.properties"; input =
			 HelperUtils.class.getClassLoader().getResourceAsStream(filename);
			 if (input == null) {
				// Log error to file
